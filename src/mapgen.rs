...
impl UpperMap {
    pub fn new(width: usize, height: usize) -> UpperMap {
        ...
        let islands = vec![Circle {
            center: Vec2::new((width / 2) as f32, (height / 2) as f32),
            radius: width as f32 / 1.8,
        }];

        println!("Generating island noise");
        let noise = Fbm::new().set_octaves(5).set_frequency(1.0 / (width as f64 / 4.5));
        let mut elevation =
            Grid::from_fn(width, height, |x, y| noise.get([x as f64, y as f64]) as f32);
        ...
        for ((x, y), tile) in
            coordinates_iter.zip(elevation.iter_mut()).filter(|&(_, &mut e)| e < 0.0)
        {
...
fn randomize_elevation(map: &mut Grid<f32>) {
    let noise = Fbm::new().set_octaves(8).set_frequency(0.02);
    let mut rand_map =
        Grid::from_fn(map.width(), map.height(), |x, y| noise.get([x as f64, y as f64]) as f32);
...
fn radial_fade(circle: Circle, point: Vec2<f32>) -> f32 {
    ...
    if dist > circle.radius { 0.0 } else { 1.0 - dist / circle.radius }
}
    ...
    T: Interpolate,
{
    Grid::from_fn(input.width() * scale, input.height() * scale, |x, y| {
        ...
        let values = [[input[(in_x, in_y)].clone(), input[(in_x, in_y + 1)].clone()], [
            input[(in_x + 1, in_y)].clone(),
            input[(in_x + 1, in_y + 1)].clone(),
        ]];
